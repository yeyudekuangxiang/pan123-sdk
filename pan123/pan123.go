...
func (p123 *Pan123) fileUploadCreateFile(parentFileID int64, filename string, file IFile, fileSize int64) (*fileUploadCreateFileRespData, error) {
	hash := md5.New()
...
func (p123 *Pan123) fileUploadChunkUpload(preuploadID string, sliceSize int64, file IFile, retry int, cb FileUploadCallbackFunc, chunkCount int64) (*fileUploadChunkUploadRespData, error) {
...
//
// @return SDKError
func (p123 *Pan123) FileUploadWithCallback(parentFileID int64, filename string, file IFile, retry int, cb FileUploadCallbackFunc) (*FileUploadRespData, error) {
	fileSize := file.Size()
	...
	createFileResp, err := p123.fileUploadCreateFile(parentFileID, filename, file, fileSize)
	if err != nil {
		return nil, err
	}
	...
	// 分块上传
	var chunkCount int64 = fileSize / createFileResp.SliceSize
	if fileSize%createFileResp.SliceSize != 0 {
		chunkCount++
	}
	...
	// 上传完毕, 进行校验
	if createFileResp.SliceSize < fileSize && len(chunkUploadResp.fileSliceSizes) > 1 {
		cb(FileUploadCallbackInfo{
...
//
// @return SDKError
func (p123 *Pan123) FileUpload(parentFileID int64, filename string, file IFile, retry int) (*FileUploadRespData, error) {
...
func (p123 *Pan123) DownloadInfo(fileID int64) (*DownInfoRespData, error) {
	querys := map[string]string{
		"fileId": strconv.FormatInt(fileID, 10),
	}

	resp, err := p123.callApi("/api/v1/file/download_info", "GET", nil, querys, true)
	if err != nil {
		return nil, err
	}

	var respData DownInfoRespData
	err = toRespData(resp.Data, &respData)
	if err != nil {
		return nil, err
	}
